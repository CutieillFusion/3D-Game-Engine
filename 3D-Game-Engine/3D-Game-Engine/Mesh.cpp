...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	if (shapes.size() > 1) throw std::exception("Mesh not created successfully");//Can currently only Handle One shape


	std::vector<VertexMesh> list_vertices;
	std::vector<unsigned int> list_indices;

	for (size_t s = 0; s < shapes.size(); s++)//Runs through the Shapes
	{
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)//Runs Through the Faces
		{
			unsigned char num_face_verts = shapes[s].mesh.num_face_vertices[f];

			for (unsigned char v = 0; v < num_face_verts; v++)//Runs Through the Vertices
			{
				//Gets Index
	...
	//Creates Temp Shader to create a Vertex and Index Buffer 
