...
void SpaceShooterGame::updateThirdPersonCamera()
{
	...
	Vector3D new_pos = m_cam_pos + world_cam.getZDirection() * (-m_current_cam_distance) + world_cam.getYDirection() * (5.0f);
...
void SpaceShooterGame::updateLight()
{
	...
	m_light_rot_matrix.setRotationY(0.707);
...
void SpaceShooterGame::onCreate()
{
	...
	for (unsigned int i = 0; i < 200; i++) 
	{
		m_asteroid_pos[i] = Vector3D(rand() % 4000 + (-2000), rand() % 4000 + (-2000), rand() % 4000 + (-2000));
	...
	m_spaceship_tex = GraphicsEngine::get()->getTextureManager()->createTextureFromFile(L"..\\3D-Game-Engine\\Assets\\Textures\\spaceship.jpg");
	m_spaceship_mesh = GraphicsEngine::get()->getMeshManager()->createMeshFromFile(L"..\\3D-Game-Engine\\Assets\\Meshes\\spaceship.obj");

	m_asteroid_tex = GraphicsEngine::get()->getTextureManager()->createTextureFromFile(L"..\\3D-Game-Engine\\Assets\\Textures\\asteroid.jpg");
	m_asteroid_mesh = GraphicsEngine::get()->getMeshManager()->createMeshFromFile(L"..\\3D-Game-Engine\\Assets\\Meshes\\asteroid.obj");
